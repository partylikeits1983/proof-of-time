...
fn main() -> io::Result<()> {
    ...
    // Read the proof file
    let mut file = File::open("../circuits/create_event/target/proof")?;
    ...
    for (idx, input_hex) in public_inputs.iter().enumerate() {
        let filename = match idx {
            0=> "create_event_timestamp.txt",
            1 => "create_event_leaf.txt",
            _ => {
    ...
    let proof_file_path = data_dir.join("create_event_proof.txt");
